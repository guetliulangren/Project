...
<script text/javascript>
	...
	function to_page(pn) {
		$.ajax({
			...
			url : "${app_context}/queryall/resposity",
			data : "pn=" + pn,
			type : "GET",
			dataType : "json",
...

	{
		...
		$("<tr></tr>").append(head1).append(head2).append(head3).append(head4)
				.append(head5).append(head6).append(head7).append(head9)
		...
		$.each(role, function(index, items) {
			...
			var dbutton = $("<button></button>").addClass(
					...
					.append("修改库存信息").attr("productsuuid", items.productsuuid);
			//为按钮添加一个attr标识当前用户名称
			var dbbutton = $("<td></td>").append(dbutton);

			/* 将解析的数据追加到tbody */
			$("<tr></tr>").append(productsID).append(productsType).append(productsPrice).append(productsNumber)
					.append(productsStatus).append(productsName).append(productstime).append(productsphone)
